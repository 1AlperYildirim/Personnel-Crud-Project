...
import { deletePersonnel, getPersonnel } from "../api/personnelApi";

type Props = {
  onEdit: (p: Personnel) => void;
};

const PersonnelList: React.FC<Props> = ({ onEdit }) => {
  ...
  return (
    <div className="personnel-table">
      ...
      <div className="personnel-body">
        ...
        {data.length === 0 && <div className="no-results">No personnel found.</div>}
      </div>
    </div>
  );
};
